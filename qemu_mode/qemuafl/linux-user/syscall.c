                           ...
                           abi_ulong target_addrlen_addr, int flags)
{
    ...
    if(hookhack) {
        ...
        fputs("[HOOK] before inet_pton!\n", bk_stdout);
        inet_pton(AF_INET, "127.0.0.1", &saddr.sin_addr);
        fputs("[HOOK] after inet_pton!\n", bk_stdout);
        host_to_target_sockaddr(target_addr, (void *)&saddr, addrlen);
        fputs("[HOOK] after host_to_target_sockaddr!\n", bk_stdout);
        if (put_user_u32(addrlen, target_addrlen_addr)) 
        {
            fputs("[HOOK] put_user_u32 failed!\n", bk_stdout);
            return -TARGET_EFAULT;
        }
        hookhack_done++;
        if(getenv("GH_DRYRUN"))
                  ...
                  char *envp[])
{
    ...
    for (argc = 0; argv[argc] != NULL; argc++) {
        ...
        fprintf(stderr, "   - arg %s\n", argv[argc]);
                // 判断当前参数是否包含 "tcp_timestamps"
        if (strstr(argv[argc], "tcp_timestamps") != NULL) {
            // 找到匹配的字符串，返回 0
            return 0;
        }
    }
